...
module.exports = class extends Command {
    ...
    async run(message, args) {
        let delMessage;
        if (!this.client.deletedMessages) {
            this.client.deletedMessages = new Map();
        }
        ...
        if (args.length === 0) {
            ...
            if (!delMessage) {
                throw new Error('Could not find any deleted messages of this channel.');
            }
        }
        else if (args.length === 1) {
            if (isNaN(args[0])) {
                throw new Error('Your channel id seems to be incorrect.');
            }
            ...
            if (isNaN(args[1])) {
                throw new Error('Your user id doesn\'t seem to be a number.');
            }

            delMessage = this.client.deletedMessages.get(args[2]);
            if (!delMessage) {
                throw new Error('Could not find any deleted messages of this user.');
            }
        } else {
